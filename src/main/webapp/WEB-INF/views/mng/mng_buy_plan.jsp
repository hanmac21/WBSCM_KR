...
<body class="hold-transition skin-blue layout-top-nav">
			...
			<section class="content">
				...
				<div class="row" style="width: 100%; margin-bottom: 5px; padding-bottom: 5px;">
					...
					<div class="col-xs-12" style="padding-right: 0px;">
						...
						%>
						<input type="hidden" name="p_cno" id="p_cno" placeholder="협력사번호"
						...
						%>
						<span>차종 : </span> &nbsp;<input type="text" id="p_carname"
						...
						&nbsp; <span>공장 : </span> &nbsp;<select id="branch" name="branch" style="height:27px;">
						 ...
						 &nbsp;&nbsp;&nbsp; <span>조회조건 : </span> &nbsp;<select
							id="alldtate" name="alldtate" style="height:27px;">
							<option value="0" ${alldtate =='0' ? 'selected' :'' }>계획품목</option>
							<option value="1" ${alldtate =='1' ? 'selected' :'' }>전체품목</option>
						</select>
						...
						&nbsp;&nbsp;&nbsp; <span>미발행분 : </span> &nbsp;<select
							id="income" name="income" style="height:27px;">
							<option value="0" ${income =='0' ? 'selected' :'' }>전체</option>
							<option value="1" ${income =='1' ? 'selected' :'' }>미발행분</option>
						</select>
						 
						&nbsp;<input type="button" class="btn btn-warning" id="search" 
							onclick="getUserList('1')" value="조회하기(새로고침)"
						...
						&nbsp;&nbsp;&nbsp; <input type="button" class="btn btn-success"
							onclick="fnMaDateSet()" value="생산일자 일괄적용"
							style="margin-left: 10x;" />
						<input type="button"
							class="btn btn-info" onclick="fnTranViewSet()" value="거래명세서발행하기"
							...
							<div class="box">
								...
								<div class="box-body" style="overflow: scroll;">
									...
									<table id="tablebody" width="100%"
										...
										<thead>
											<tr>
												<th style="text-align: center; width: 3%;">순번</th>
												<th style="text-align: center; width: 7%;">협력사<br>(협력사코드)</th>
												<th style="text-align: center; width: 25%;">품번<br>(품명)</th>
												<th style="text-align: center; width: 5%;">차종</th>
												...
												<th style="text-align: center; width: 5%;">합계</th>
											</tr>
										</thead>
										...
										<tbody>
											<c:choose>
												...
												<c:otherwise>
													<tr>
														<td colspan="19" style="height: 100px; text-align: center;">검색된 자료가없습니다.</td>
													</tr>
												</c:otherwise>
	...
	<div class="" style="padding-top: 0px; margin-right: 10px; width: 33%; float: left;">
		<span style="margin-left: 10px;">건수1 : ${ itemCount }</span>
	</div>
	...
	function fnNDeliverySet(){
		...
		function fnclose2(){
			//fnStartLabel();
			//	alert('a');
			if(confirm("지금 발행하지 않으시면 추후에 전산문제가 발생할 수 있습니다. 정말 취소하시겠습니까?")){
				$('#modalPop3').modal('show'); //거래명세서 화면을 가린다.
		...
		function fnLabelPrint() {
			...
			if(qty!=Number(production)+Number(production2)+Number(production3)){
				...
				alert("주문 수량과 LOT수량의 합이 일치하지 않습니다.");
				$('#p_production').focus();
				return false;
			}
			//alert ("메모내용=>"+memo);
			if((Number(box_qty)+Number(box_qty2)+Number(box_qty3))==0){
				alert("박스당 입수수량을 입력해주세요");
			$('#p_box_qty').focus();
				return false;
			}
			if(production!=0){
